...
@Component({
  ...
  template: `
    <!-- Header -->
    <div class="flex items-start justify-between px-7 pt-6 pb-4 border-b border-slate-100">
      <div class="flex items-center gap-3 min-w-0">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-600 to-slate-800 flex items-center justify-center shadow-lg shadow-slate-500/20 flex-shrink-0">
          <span class="material-icons text-white text-[20px]">visibility</span>
        </div>
        <div class="min-w-0">
      ...
      <button mat-dialog-close class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all flex-shrink-0">
    ...
    <mat-dialog-content>
        ...
        @for (opt of q.options; track $index; let i = $index) {
          <div class="flex items-start gap-3 p-3.5 rounded-xl border"
            ...
            @if (q.correctOptionIndices.includes(i)) {
              <span class="material-icons text-emerald-500 text-[18px] flex-shrink-0">check_circle</span>
            }
          </div>
        }
      </div>
      ...
      <!-- Metadata -->
      <div class="grid grid-cols-2 gap-x-6 gap-y-3 pt-4 border-t border-slate-100 text-xs">
        <div>
